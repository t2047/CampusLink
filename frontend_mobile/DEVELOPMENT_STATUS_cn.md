...
>
> 当前阶段：Android Mail / Calendar 已补齐邮件日程扫描范围与来源邮件联动；提交/PR、物理真机与 Agent 写操作专项仍待完成
...
- 日历邮件联动：支持扫描今天、近 7 天或近 30 天的邮件，最多返回 50 条候选，避免默认只扫描当天导致漏掉旧邮件中的会议通知；
...
- 日历月视图/周视图、日期时间选择器，目前使用 ISO 文本输入和列表视图；时间字符串已支持带时区显示，但仍缺少可视化的时区选择；
...
- 下一阶段：日历月/周视图与日期时间选择器、自然语言创建/修改日程的确认流程、附件、草稿、批量操作、离线缓存、推送通知、平板/横屏和更高动态字体专项。
